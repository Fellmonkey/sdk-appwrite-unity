...
public class CreateSessionExample : MonoBehaviour
{
    ...
    async UniTask ExampleCreateSession()
    {
        try
        {
            ...
            var secret = "<SECRET>"; // Secret of a token generated by login methods. For example, the `createMagicURLToken` or `createPhoneToken` methods.
            
            var result = await client.Account.CreateSessionAsync(
                userId,
                secret
            );
